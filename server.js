...
async function sendOnboardingEmail(username, email, stationIds) {
  try {
...
// API endpoint to send onboarding email
app.post('/api/send-onboarding-email', async (req, res) => {
  try {
    ...
    // Get user data to get station IDs
    const user = await db.getUserByUsername(username);
    if (!user) {
    ...
    const loginUrl = `${req.protocol}://${req.get('host')}/login`;
    const htmlContent = generateOnboardingEmailHTML(username, user.station_ids || [], loginUrl);
